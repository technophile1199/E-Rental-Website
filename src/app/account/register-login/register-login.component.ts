...
export class RegisterLoginComponent implements OnInit {
  ...
  public onRegister() {
    if (this.registerForm.value.password !== this.registerForm.value.confirmPassword) {
      ...
      this.authenticationService.emailSignUp(this.registerForm.value.email, this.registerForm.value.password, this.registerForm.value.firstName, this.registerForm.value.lastName, this.registerForm.value.phoneNumber)
      .then(
        () => {
        ...
        (error) => {
          this.registerErrors = "I Am "+error.message;
      ...
      .then(
        ...
        (error) => {
          ...
          if (error.code === 'auth/wrong-password') {
            ...
            this.messageService.addError('Login Failed!!');
            console.log("Error 3 : "+error);
            console.log("Error 3_1 : "+error.message);
          }
        }
      );
  }
  ...
  {
    this.authenticationService
    .phoneLogin(this.loginForm.value.phoneNumber, this.loginForm.value.password) 
    .then(
      () => {
        this.messageService.add('Login successful!');
        this.router.navigate(['/home']);
