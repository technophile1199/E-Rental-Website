...
export class ProductService {
  ...
  public getSeller() {
    const items = this.authService.user
      .pipe(
        switchMap((user) => {
          if (user) {
            const remoteUserOrders = `/products_2`;
  ...
  public getp() {
    return this.authService.user
      .pipe(
        switchMap((user) => {
          if (user) {
            const remoteUserOrders = `/products`;
    ...
    const dbOperation = this.uploadService
      ...
      .then((task) => {
        task.ref.getDownloadURL().then(url=>{
          ...
          data.product.imageRefs.push(task.ref.fullPath.toString());
  
          this.angularFireDatabase
          .list(`products_2/${firebase.auth().currentUser.uid}/product`).update(data.product.id.toString(),data.product);

          firebase.database().ref(`products_3/${data.product.id.toString()}/seller`).update({
  ...
  public deleteProduct(product: Product) {
    ...
    this.angularFireDatabase
      .list(`/products_2/${firebase.auth().currentUser.uid}/product/${product.id}`)
      .remove()
      .then(() => this.log(''))
      .catch((error) => {
